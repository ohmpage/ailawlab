...
Fairness Explorer V2 + V3. V2: four-slide tutorial carousel; Print / Export button (opens new window, triggers browser print). V3: dataset switcher with four real-world datasets — COMPAS by Race (default), COMPAS by Gender, Gender Shades (IBM Watson — Buolamwini & Gebru FAccT 2018), Emily & Greg (Bertrand & Mullainathan AER 2004). Positive Rate (PP/all) added as 11th metric, linking to Title VII disparate impact and the EEOC four-fifths rule. Research footer expanded with primary sources for all four datasets. UI polish: slide title clears Skip button; instruction hint repositioned between matrices and workspace.
