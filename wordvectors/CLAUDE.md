...
### Version 6 — June 2026
**Shared header.** The header is now generated by `shared/js/header.js` from a per-page `window.APP_HEADER` config. Left side: app title + "by Paul Ohm" byline (replacing the former "Vibe Coded by Paul Ohm"). Right side: Tutorial link and All Apps link. `window.restartTutorial()` is defined in `app.js` — it calls `renderSlide(0)` and restores the explainer section / hides the demo section, letting students return to the slides at any time.
