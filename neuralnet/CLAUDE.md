...
### Version 4 — June 2026
**Shared header.** The header is now generated by `shared/js/header.js` from a per-page `window.APP_HEADER` config. Left side: app title + "by Paul Ohm" byline. Right side: Tutorial link (calls `window.restartTutorial()`) and All Apps link. The "Take a tour" link and the hamburger menu (☰) were removed from the header.

**Controls-bar relocation.** The hamburger (Network Settings) and "Take a tour" link now live at the left and right ends of the controls bar respectively — both are hidden until a network is built, which is also when they're needed. The controls-bar `z-index` was raised from 10 to 20 so it paints above the settings panel; the hamburger remains clickable even when the panel is open, allowing users to dismiss the panel without building.

**Tutorial restart.** `window.restartTutorial()` is defined in `app.js` — it ends any running guided tour, resets `slideIndex` to 0, re-renders slide 1, and hides the app to show the explainer. The Tutorial link in the header calls this.
