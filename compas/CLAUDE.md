...
### Version 3 — June 2026
**Wand slider persistence.** All three Magic Wand interventions now preserve their slider positions across Reality ↔ Wand mode toggles. Previously `enterWandMode()` was resetting `thresholdBlack`, `thresholdWhite`, and `accuracy` to their defaults on every entry. Now only the Defaults button resets them. The `realBaseRateBlack`/`realBaseRateWhite` immutable pair continues to anchor what "Defaults" means for the base rate sliders.

**Shared header.** The header is now generated by `shared/js/header.js` from a per-page `window.APP_HEADER` config. Left side: "COMPAS Risk Score Explorer" + "by Paul Ohm" byline. Right side: Tutorial link and All Apps link. `window.restartTutorial()` resets `slideIndex` to 0 and returns to the four-slide explainer, hiding the main dashboard.
